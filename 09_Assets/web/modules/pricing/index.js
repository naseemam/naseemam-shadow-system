(function () {
      ...
      render(container) {
        ...
        container.querySelector('[data-action="ask-page"]')?.addEventListener('click', () => {
          if (window.AmeerWorkspaceShell && typeof window.AmeerWorkspaceShell.sendPrompt === 'function') {
            window.AmeerWorkspaceShell.sendPrompt('أعطني ملخصًا سريعًا عن صفحة pricing وما الذي تفعله الآن.');
          }
        });
        container.querySelector('[data-action="improve-page"]')?.addEventListener('click', () => {
          if (window.AmeerWorkspaceShell && typeof window.AmeerWorkspaceShell.sendPrompt === 'function') {
            window.AmeerWorkspaceShell.sendPrompt('اقترح تحسينات عملية لصفحة pricing ثم ابدأ بتنفيذ أول تحسين آمن داخل الواجهة.');
          }
        });
  ...
  window.AmeerWorkspaceModules['pricing'] = createPricingModule();
})();
