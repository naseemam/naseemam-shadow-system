(function () {
      ...
      render(container) {
        ...
        container.querySelector('[data-action="ask-page"]')?.addEventListener('click', () => {
          if (window.AmeerWorkspaceShell && typeof window.AmeerWorkspaceShell.sendPrompt === 'function') {
            window.AmeerWorkspaceShell.sendPrompt('أعطني ملخصًا سريعًا عن صفحة services وما الذي تفعله الآن.');
          }
        });
        container.querySelector('[data-action="improve-page"]')?.addEventListener('click', () => {
          if (window.AmeerWorkspaceShell && typeof window.AmeerWorkspaceShell.sendPrompt === 'function') {
            window.AmeerWorkspaceShell.sendPrompt('اقترح تحسينات عملية لصفحة services ثم ابدأ بتنفيذ أول تحسين آمن داخل الواجهة.');
          }
        });
  ...
  window.AmeerWorkspaceModules['services'] = createServicesModule();
})();
