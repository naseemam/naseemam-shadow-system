(function () {
      ...
      render(container) {
          ...
          <section class="module-card" style="display:grid;gap:12px;">
            <div class="dashboard-hero">
              <div>
                ...
                <h2>about</h2>
                <p>أضفها للموقع وحدث التنقل ثم أخبرني بما فعلت</p>
              </div>
        ...
        container.querySelector('[data-action="ask-page"]')?.addEventListener('click', () => {
          if (window.AmeerWorkspaceShell && typeof window.AmeerWorkspaceShell.sendPrompt === 'function') {
            window.AmeerWorkspaceShell.sendPrompt('أعطني ملخصًا سريعًا عن صفحة about وما الذي تفعله الآن.');
          }
        });
        container.querySelector('[data-action="improve-page"]')?.addEventListener('click', () => {
          if (window.AmeerWorkspaceShell && typeof window.AmeerWorkspaceShell.sendPrompt === 'function') {
            window.AmeerWorkspaceShell.sendPrompt('اقترح تحسينات عملية لصفحة about ثم ابدأ بتنفيذ أول تحسين آمن داخل الواجهة.');
          }
        });
  ...
  window.AmeerWorkspaceModules['about'] = createAboutModule();
})();
