...
def test_only_delete_and_production_delivery_are_founder_gate_actions():
    assert "delete" in _HIGH_RISK_ACTIONS_REQUIRING_APPROVAL
    ...
    assert "rollback" in _HIGH_RISK_ACTIONS_REQUIRING_APPROVAL


def test_approval_gate_is_limited_to_delete_and_production_delivery():
    assert "delete" in ApprovalGate.HIGH_RISK_ACTIONS
    assert "publish" in ApprovalGate.HIGH_RISK_ACTIONS
    assert "deploy" in ApprovalGate.HIGH_RISK_ACTIONS
    assert "rollback" in ApprovalGate.HIGH_RISK_ACTIONS
    assert "external" not in ApprovalGate.HIGH_RISK_ACTIONS
    assert "financial" not in ApprovalGate.HIGH_RISK_ACTIONS
