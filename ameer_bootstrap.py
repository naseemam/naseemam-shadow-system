...
def _build_repository_kernel() -> ExecutiveKernel:
    ...
    # Replace the runtime-only file.create permission with an explicit,
    # controlled repository scope. The policy still denies secrets, .git,
    # .github, backups, traversal, and all unspecified paths.
...
# Persistent conversation/project data can continue to use AMEER_DATA_DIR.
# Only the Executive Kernel is rebound to the actual repository checkout so
# controlled file execution modifies deployable code rather than the volume.
